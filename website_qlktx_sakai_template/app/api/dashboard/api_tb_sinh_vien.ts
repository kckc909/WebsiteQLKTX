...
export async function api_tb_sinh_vien_add(sinh_vien: Partial<tb_sinh_vien>) {
...
export async function api_tb_sinh_vien_CheckByMSV(msv: string) {
    const { data } = await axios.get(`${apiUrl}/checkByMSV/${msv}`);
    return data;
}